...
class Cliente extends Entidad {
    ...
    public function __construct($adapter) {
        parent::__construct("clientes", $adapter);
    }
    ...
    public function guardar(){
        ...
        $per = $this->traerCrearPersona($this->getNombres(), 
            ...
            $this->getCorreo()); 
        $pid = (int)$per; 
        $query="INSERT INTO clientes (id,persona) VALUES (NULL,".$pid.");";
    ...
    public function traerCrear($nom, $ced, $tel, $dir, $cor) {        
        ...
        $sql->execute(); 
        $res = $sql->fetchAll(); 
        if (count($res) <= 0) { // Persona no encontrada: Crear persona y cliente
            // 1: Crer persona 
            ...
            $pid = $res["id"];
            // 2: Crear cliente
            $query="INSERT INTO clientes (persona) VALUES (" . $pid . ");";
            $this->db()->query($query);
            $qid = $this->db()->prepare("Select last_insert_id() as id;"); 
            $qid->execute(); 
            $res = $qid->fetch(); 
            $id = $res["id"];
        } else { // Persona encontrada: Buscar si esta como cliente
            $idc = 0;
            $fila = reset($res);  
            if ($fila != false) 
                $idc = $fila["id"];
            $sql = $this->db()->prepare("SELECT id FROM clientes WHERE persona = " . $idc . " LIMIT 1"); 
            $sql->execute(); 
            $resc = $sql->fetchAll(); 
            if (count($res) <= 0) {
                // Se encontro persona pero no cliente: 
                // Insetar cliente y retornar su id
                $query="INSERT INTO clientes (persona) VALUES (" . $id . ");";
                $this->db()->query($query);
                $qid = $this->db()->prepare("Select last_insert_id() as id;"); 
                $qid->execute(); 
                $res = $qid->fetch(); 
                ...
                // Se encontro persona en base de cliente: retornar id encontrado
                $filac = reset($resc);  
                if ($filac != false) 
                    $id = $filac["id"];
            }
    ...
    public function traerPaginadoClis($idx, $conteo) {
        $sql = $this->db()->prepare("SELECT c.id, c.persona, p.nombres, p.cedula, p.direccion, p.telefonos, p.correo, p.estado FROM clientes c " .
