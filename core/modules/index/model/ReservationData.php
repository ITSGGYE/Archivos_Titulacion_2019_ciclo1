...
class ReservationData {
	...
	public function add(){
		...
		echo $sql .= "values (\"$this->title\",\"$this->note\",\"$this->medic_id\",\"$this->date_at\",\"$this->time_at\",$this->pacient_id,$this->user_id,$this->created_at)";
...
// partiendo de que ya tenemos creado un objecto ReservationData previamente utilizamos el contexto
	public function update(){
        ...
        public function hc_insert(){
            $sql = "update ".self::$tablename." set descripcion=\"$this->descripcion\",estado=\"$this->estado\" where id = $this->id";
            Executor::doit($sql);
	...
	public static function getAll(){
		$sql = "select * from ".self::$tablename." where date(date_at)>=date(NOW()) AND estado = 1 order by date_at";
		$query = Executor::doit($sql);
