...
$cedula = $_POST['cedula'];
$query = "SELECT * FROM estudiantes WHERE identificador = :_CEDULA ";
$stmt = $conexion->prepare($query);
$stmt->bindParam(':_CEDULA',$cedula,PDO::PARAM_STR,4000);
$stmt->execute();
$result = $stmt->fetch();
if($result['identificador'] == $cedula ){
date_default_timezone_set('America/Guayaquil');
$fecha = date('Y-m-d');
$conexion = Conexion::obtenerConexion();
$query = "SELECT id,nombres,apellidos FROM estudiantes WHERE identificador = :_cedula";
$stmt = $conexion->prepare($query);
$stmt->bindParam(':_cedula',$cedula,PDO::PARAM_STR,4000);
$stmt->execute();
$result = $stmt->fetch();
