...
<body>
	...
	<section id="container">
		...
		<form action="buscar_paciente.php" method="POST" class="form_search">
			<input type="text" name="cedula" id="cedula" placeholder="Buscar" required>
		...
		<?php
			//Paginador
			$sql_registe = mysqli_query($conection,"SELECT COUNT(*) as total_registro FROM personas WHERE type_persona = 3 AND estatus = 1 ");
			...
			$query = mysqli_query($conection,"SELECT * FROM personas
			   WHERE type_persona = 3 AND estatus = 1  ORDER BY id ASC LIMIT $desde,$por_pagina
				");
