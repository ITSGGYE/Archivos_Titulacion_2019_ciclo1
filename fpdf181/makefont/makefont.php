...
{
	// Devuelve información de una fuente TrueType
	try
	{
...
{
	// Devuelve información de una fuente Type1
	if($embed)
	{
		$f = fopen($file, 'rb');
		if(!$f)
...
// Crea diferencias con la codificación de referencia
	$ref = LoadMap('cp1252');
	$s = '';
	$last = 0;
	for($c=32;$c<=255;$c++)
...
// Construir mapeo a valores set de caracteres universal
	$ranges = array();
	foreach($map as $c=>$v)
	{
		$uv = $v['uv'];
...
// Genera un archivo de definición de fuente
