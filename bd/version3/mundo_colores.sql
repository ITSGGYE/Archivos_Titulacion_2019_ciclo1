...
create table cobro_matricula(
id_cobromatricula int not null auto_increment,
fk_alumno int not null,
fk_cobro  int  not null,
detalle varchar(150) null,
fechareg date,
foreign key (fk_alumno) references alumno (id_alumno),
foreign key (fk_cobro) references cobro_valores (id_cobro),
primary key (id_cobromatricula));

select * from cobro_matricula;
...
select * from cobro_pensiones;


create table cobro_valores(
id_cobro int not null auto_increment,
...
truncate table cobro_matricula;




SELECT 	p.nombre_repre, a.direc_alumno, p.celular_repre, p.cedula_repre, a.nombre_alumno,co.fechareg, c.nombre_curso, co.id_cobromatricula FROM alumno a, curso_alumno cr, curso c, cobro_matricula co, datos_representante p  where   a.id_alumno=co.fk_alumno and
		cr.fk_alumno=co.fk_alumno and cr.fk_curso=c.id_curso and co.fk_alumno=p.fk_alumno;
