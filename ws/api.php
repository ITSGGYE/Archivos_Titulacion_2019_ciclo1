    ...
    switch($_GET['action']){
        ...
        if(isValid(array('username','email','password'))) {
            ...
            if($stmt->num_rows == 0) {
                ...
                if($stmt->execute()){
                    $stmt = $conn->prepare("SELECT id, username, email, type FROM users WHERE username = ?"); 
		...
		'type'=>$type
                ...
                $response['error'] = true; 
                $response['message'] = 'Invalid username or password user:'.$username.'  password:'.$password;
            }
        } else {
            $response['error'] = true; 
