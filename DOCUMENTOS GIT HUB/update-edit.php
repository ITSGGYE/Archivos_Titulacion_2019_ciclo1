...
if (isset($_POST['update'])) {
    ...
    if ($update) {
        if($_SESSION['rol'] == 1){
            echo "<script>alert('Los datos han sido actualizados!'); window.location = 'act_historial.php'</script>";
       ...
       if($_SESSION['rol'] == 1){
            echo "<script>alert('Error al actualizar!'); window.location = 'act_historial.php'</script>";
        }elseif($_SESSION['rol'] == 2){
            echo "<script>alert('Error al actualizar!'); window.location = 'act_historial_vet.php'</script>";
        }
    }
}
?>