...
<div class="box-body">
  ...
  <?php  
        $ps = mysqli_query($conection,"SELECT * FROM publicaciones");
  ...
  ?>
        ...
        <td><?php echo $poss['fecha']; ?></td>
         <td><?php echo $poss['contenido']; ?></td> 
           <td>
            <form method="post" action="edino.php" >
...
function pregunta(){
    if (confirm('¿Estas seguro que deseas eliminar este formulario?')){
       document.elimina.submit()
          ...
          <td>
            <form name="elimina" method="post" action="php/not.php">
<input type="hidden" name="id" value="<?php echo $poss['id_pub']; ?>">
